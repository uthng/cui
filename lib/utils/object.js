...
export default {
  ...
  removeObjectByPath: function (obj, sep = '.', path) {
    ...
    var isArray = (a.last().match(/^\d+$/))
    var extent = (isArray) ? a.size() - 2 : a.size() - 1
    ...
    if (match !== null) {
      ...
      if (isArray) {
        context[a[a.size() - 2]] = context[a[a.size() - 2]].filter(function (x, itt) {
          return itt !== Number(a.last())
        })
      } else {
        delete context[a.last()]
      }
    }
