...
ACL.prototype.read = function(uuid, token = "") {
  var url =
    this.apiUrl + "/info/" + uuid + (token !== "" ? "?token=" + token : "")

  return axios.get(url, { crossDomain: true }).catch(e => {
    console.log(e)
  })
}
...
ACL.prototype.create = function(acl, token = "") {
  ...
  return axios.put(url, newAcl, { crossDomain: true }).catch(e => {
    console.log(e)
  })
}
...
ACL.prototype.update = function(acl, token = "") {
  ...
  return axios.put(url, acl, { crossDomain: true }).catch(e => {
    console.log(e)
  })
}
...
ACL.prototype.delete = function(uuid, token = "") {
  var url =
    this.apiUrl + "/destroy/" + uuid + (token !== "" ? "?token=" + token : "")

  return axios.put(url, {}, { crossDomain: true }).catch(e => {
    console.log(e)
  })
}
...
ACL.prototype.clone = function(uuid, token = "") {
  var url =
    this.apiUrl + "/clone/" + uuid + (token !== "" ? "?token=" + token : "")
